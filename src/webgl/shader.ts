...
export function createShaderProgram(vertexSource: string, fragmentSource: string) {
	const vertexShader = compileShader(GL_VERTEX_SHADER, vertexSource);
	const fragmentShader = compileShader(GL_FRAGMENT_SHADER, fragmentSource);
...
export function compileShader(type: ShaderType, source: string) {
